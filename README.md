...
나쁜 곳에 있다"는 **상시 환경 등급**입니다. 기간이 없고 바꾸기 전까지 유지됩니다. FE의 등급
배지가 기기마다 갈리게 해 상태 다양성을 만드는 것이 목적입니다.
...
값은 `base±amplitude`(일주기 진폭)입니다. 프리셋은 **차등 테이블**이라 명시한 지표만 바뀌고
나머지는 `good`을 물려받습니다. 노이즈·피크 시각·**min/max 클램프·자릿수는 손대지 않으므로**
업로드 검증(422) 계약이 그대로입니다. `moderate`는 등급 경계를 걸치도록 잡아 시간대에 따라
'좋음'과 '나쁨' 사이를 오갑니다. 생체는 `bad`부터만 미세하게 반응합니다.
